...
def prepare_hebrew_text(text):
    """
    עיבוד מלא לעברית עם RTL-Embedding (תיקון מוחלט לכל שורה)
    """
    try:
...
def wrap_text(text, font, max_width, draw):
    ...
    for word in words:
        ...
        test_line_rtl = prepare_hebrew_text(test_line)
        try:
            bbox = draw.textbbox((0, 0), test_line_rtl, font=font)
            width = bbox[2] - bbox[0]
        except Exception:
            width = draw.textsize(test_line_rtl, font=font)[0]
        if width <= max_width:
    ...
    if current_line:
        ...
        lines.append(prepare_hebrew_text(line_text))
    if not lines:
        lines = [prepare_hebrew_text(text)]
    return lines
